...
const handleSubmit = async (e) => {
    ...
    e.preventDefault();
    try {
      const response = await axios.post("https://backend-portalturismo-67uj.onrender.com/api/auth/login", {
        email,
        password
      })
        ...
        >
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
                ...
                <div className="text-center"> {/* botão para enviara mensagem */}
                    <button
                        type="submit"
                        className="bg-violet-500 hover:bg-violet-600 text-white px-6 py-2 rounded-lg transition duration-300 w-full sm:w-full"
                    >
                        Enviar Mensagem
                    </button>
                </div>
