...
const Navbar = () => {
    ...
    const menuLinks = (
        <>
            {/* Links públicos */}
            <a href="/" className="text-white hover:text-pink-300 transition-colors duration-300 text-lg font-medium">Home</a>
            <a href="/about" className="text-white hover:text-pink-300 transition-colors duration-300 text-lg font-medium">Sobre</a>
            <a href="/service" className="text-white hover:text-pink-300 transition-colors duration-300 text-lg font-medium">Serviços</a>
            <a href="/contact" className="text-white hover:text-pink-300 transition-colors duration-300 text-lg font-medium">Contato</a>
 
            {/* Se o usuário estiver autenticado mostra perfil e botão sair */}
            {isAuthenticated ? (
                <>
                    
                    <button onClick={handleLogout} className="text-white hover:text-pink-300 transition-colors duration-300 text-lg font-medium">Sair</button>
                </>
            ) : (
                // Se não estiver autenticado, mostra o link para login
                <a href="/login" className="text-white hover:text-pink-300 transition-colors duration-300 text-lg font-medium">Login</a>
            )}
        </>
    );
    ...
    return (
        ...
        <nav className="w-full bg-gradient-to-r from-violet-600 to-pink-200 text-gray-800 py-3 px-2 shadow-lg fixed left-0 top-0 height-100% width-200px  z-30 ">
            {/* Container interno com limite máximo de largura e flexbox para espaçar os itens */}
            <div className="max-w-7xl mx-auto flex justify-between items-center">
               
                {/* Logo/título do site */}
                <div className="flex items-center">
                    <a href="/" className="ml-[7.5px] text-2xl font-extrabold text-white hover:text-pink-100 transition-colors duration-300">
                        Nova Serra
                    </a>
                </div>
            ...
            <div className={`lg:hidden p-4 transition-all duration-300 ${isMenuOpen ? 'block' : 'hidden'}`}>
                {menuLinks}
            </div>
        </nav>
    );
